...
def profile(request, slug, *args, **kwargs):
    ...
    # Corrigido: campo student é User, não SchoolUser
    ...
    return render(request, themed_template(school, "profile"), context)
...
def profile_update(request, slug, school_user, school):
    """
    Atualiza informações pessoais do perfil
    """
    if request.method == 'POST':
        user = request.user
...
def profile_banner_upload(request, slug, school_user, school):
    ...
    if request.method == 'POST' and request.FILES.get('banner'):
        ...
        profile.save()
        return JsonResponse({'success': True, 'banner_url': profile.banner.url})
